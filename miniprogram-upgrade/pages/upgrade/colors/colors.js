// pages/upgrade/colors/colors.js
const { COLORS_DATA } = require('../../../utils/data')
const { speakWord, shuffle, vibrateShort } = require('../../../utils/util')
Page({
  data: { words:[], mode:'learn', themeColor:'#a18cd1', selectedWord:{}, score:0, qIndex:0, qTotal:10, question:{}, options:[], answered:false, picked:{}, quizType:'word2cn' },
  onLoad() {
    this.setData({ words: COLORS_DATA.map(w => ({ ...w, color: w.hex, learned:false, selected:false })) })
    this.generateQuiz()
  },
  ...
  generateQuiz() {
    const pool=shuffle(COLORS_DATA); const q=pool[0]; const opts=shuffle([q,...pool.slice(1,4)])
