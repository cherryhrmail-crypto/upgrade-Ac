// pages/upgrade/greetings/greetings.js
const { GREETINGS_DATA } = require('../../../utils/data')
const { speakWord, shuffle, vibrateShort } = require('../../../utils/util')
Page({
  data: { words:[], mode:'learn', themeColor:'#667eea', selectedWord:{}, score:0, qIndex:0, qTotal:10, question:{}, options:[], answered:false, picked:{}, quizType:'word2cn' },
  onLoad() {
    this.setData({ words: GREETINGS_DATA.map(w => ({...w, color:'#764ba2', learned:false, selected:false})) })
    this.generateQuiz()
  },
  ...
  generateQuiz() {
    const pool=shuffle(GREETINGS_DATA); const q=pool[0]; const opts=shuffle([q,...pool.slice(1,4)])
