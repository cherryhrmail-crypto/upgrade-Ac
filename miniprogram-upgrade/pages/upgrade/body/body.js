// pages/upgrade/body/body.js
const { BODY_DATA } = require('../../../utils/data')
const { speakWord, shuffle, vibrateShort } = require('../../../utils/util')
Page({
  data: { words:[], mode:'learn', themeColor:'#FFD93D', selectedWord:{}, score:0, qIndex:0, qTotal:10, question:{}, options:[], answered:false, picked:{}, quizType:'word2cn' },
  onLoad() {
    this.setData({ words: BODY_DATA.map(w => ({...w, color:'#FF6B35', learned:false, selected:false})) })
    this.generateQuiz()
  },
  ...
  generateQuiz() {
    const pool=shuffle(BODY_DATA); const q=pool[0]; const opts=shuffle([q,...pool.slice(1,4)])
