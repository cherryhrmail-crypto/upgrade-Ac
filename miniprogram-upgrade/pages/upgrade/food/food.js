// pages/upgrade/food/food.js
const { FOOD_DATA } = require('../../../utils/data')
const { speakWord, shuffle, vibrateShort } = require('../../../utils/util')
Page({
  data: { words:[], mode:'learn', themeColor:'#f093fb', selectedWord:{}, score:0, qIndex:0, qTotal:10, question:{}, options:[], answered:false, picked:{}, quizType:'word2cn' },
  onLoad() {
    this.setData({ words: FOOD_DATA.map(w => ({...w, color:'#f5576c', learned:false, selected:false})) })
    this.generateQuiz()
  },
  ...
  generateQuiz() {
    const pool=shuffle(FOOD_DATA); const q=pool[0]; const opts=shuffle([q,...pool.slice(1,4)])
